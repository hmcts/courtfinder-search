...
from search.court_search import Postcode, CourtSearchInvalidPostcode, \
    CourtSearchError
...
class PostcodeTestCase(TestCase):
    mock_mapit_partial = (
        '{"wgs84_lat": 51.47263752259685, "wgs84_lon": -0.06603088421009512, '
        '"postcode": "SE15" }')
    mock_mapit_full = """
        {
            "postcode": "SE15 4UH",
    ...
    def setUp(self):
        mock_get = mock.Mock(side_effect=self._get_from_mapit_mock)
        self.patcher = mock.patch('requests.get', mock_get)
        self.patcher.start()
    ...
    def test_broken_postcode(self):
        self.assertRaises(
            CourtSearchInvalidPostcode, Postcode, self.broken_postcode)
    ...
    def test_nowhere_postcode(self):
        with self.assertRaises(CourtSearchInvalidPostcode):
            Postcode(self.nowhere_postcode)
