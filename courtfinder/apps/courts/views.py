...
def format_court(court):
    ...
    for address in addresses:
        address_obj = {
            'address_lines': filter(None, address.address.split('\n')),
    ...
    emails = [
        {'description': email.description, 'addresses': [email.address]}
        for email in court.emails.all()]
    emails.sort(key=lambda x: x['description'])
    contacts = [
        {'name': contact.name, 'numbers': [contact.number]}
    ...
    court_obj = {
        ...
        'opening_times': sorted(
            [opening_time for opening_time in court.opening_times.all()],
        ...
        'alert':
            court.alert if court.alert and court.alert.strip() != '' else None,
...
def list_format_courts(courts):
    return [{
        ...
        'numbers': ', '.join(filter(None, (
            '#' + str(court.number) if court.number else None,
            'CCI: '+str(court.cci_code) if court.cci_code else None)))
        ...
        'courts':
            list_format_courts(
                Court.objects.filter(
                    name__iregex=r'^'+first_letter).order_by('name'))
            if first_letter else None
    })
