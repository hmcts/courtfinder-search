...
def courts_export(request):
    ...
    writer.writerow(['name', 'open', 'updated', 'address', 'areas of law', 'url', 'facilities'])
    for c in models.Court.objects.order_by('name').all():
        ...
        row = [c.name, 'open' if c.displayed else 'closed', str(updated), address, aols, url, facilities]
        writer.writerow(row)
    return response
